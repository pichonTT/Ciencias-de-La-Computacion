...
public class CodificadorMensajes
{
    ...
    private int[] generarCodigoEncripcion(String str) {
        if(str == null){
            throw new IllegalArgumentException("No se puede encriptar una cadena nula");
        }
        int suma = 0;//se inicializa en 0 para ir sumando en el caracter por caracter
        //el siguiente suma caracter a caracter de str y finaliza una vez el indice es mayor al tamaño 
        for(int i = 0; i < str.length(); i++){
            //a medida que se obtiene cada caracter, se hace un casting para pasarlo a su valor en ASCII
            //luego se van sumando hasta que i sea mayor a la longitud de la cadena de caracteres
            suma += (int) str.charAt(i);
        }
        int codigo = suma % 99991;// Calcula el valor temporal de la encriptación
        //otra forma de obtener un dato apropiado para la inicializacion del arreglo 
        String temporalStr = Integer.toString(codigo); // Convierte codigo a String
        int digitos = temporalStr.length(); // Obtiene la cantidad de dígitos en codigo
        codigoEncripcion = new int[ digitos ];//se inicializa segun el tamaño de la cadena de caracteres
        
        //asi se almacenara el codigo en el orden original dentro del arreglo
        for(int i = digitos - 1; i >= 0; i--){//decrementa para agarrar de atras para adelante
            codigoEncripcion[i] = codigo % 10;// Obtiene el último dígito de codigo
            codigo /= 10;// Elimina el último dígito de codigo
            
        }
        // devuelve el codigo encriptado
        return codigoEncripcion;
    }
    ...
    private String encriptarCadena(String str, int[] codigo) {
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            // es para que no se pase de los valores ascii
