...
public class DecodificadorMensajes
{
    ...
    {
        if(mensajeDecodificado != null){//Precondicion: El mensaje no debe estar decodificado
         throw new IllegalStateException("El mensaje ya está decodificado");}
        mensajeDecodificado = new Mensaje();
        for(int i = 0 ; i < mensajeADecodificar.cantLineas(); i++){//ciclo el cual desencripta el mensaje
            String act = mensajeADecodificar.obtenerLinea(i);
            String actCodificada = desencriptarCadena(act, codigoEncripcion);
            mensajeDecodificado.agregarLinea(actCodificada);
        }        
    ...
    private String desencriptarCadena(String str, int[] codigo) {
        if (str == null){//Precondicion: El mensaje es nulo
            throw new IllegalArgumentException("Mensaje nulo");}
        if (codigo == null){//Precondicion: El codigo de encriptacion es nulo
            throw new IllegalArgumentException("Código inválido.");}
        String cadenaDesencriptada = ""; //crea una variable string vacia 
        int indiceCodigo = 0; //crea una variable indice en 0
        //recorre hasta que se acabe el tamaño de str
        for(int i = 0; i < str.length() ; i++ ){
            char caracterActual = str.charAt(i);
            //Cuando el caracter tenga 0 como valor en ASCCI, cambia su valor a 127
            //decrementa 
            for(int j= codigo[indiceCodigo];j != 0;j--){
                if(caracterActual == 0){
                    caracterActual = 127;
                }
                else{
                    //si no llega a 0 le sigue restando
                    caracterActual = (char) (caracterActual - 1); 
                }
            }
            cadenaDesencriptada = cadenaDesencriptada + caracterActual;
            indiceCodigo = ((indiceCodigo + 1)% codigo.length);
        }
        return cadenaDesencriptada;//retorna el codigo desencriptado
    }
